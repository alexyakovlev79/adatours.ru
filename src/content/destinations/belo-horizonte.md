---
id: destination_brazil_belo_horizonte
locale: ru
name: Белу-Оризонти
slug: belo-horizonte
status: approved
countryId: country_brazil
destinationType: city
summary: >-
  Белу-Оризонти, столица штата Минас-Жерайс, дает современную городскую часть маршрута и удобную точку для поездок по историческому региону. В самом городе основной архитектурный сюжет связан с Оскаром Нимейером и Пампульей.
hero:
  src: https://brasiltours.ru/image/countries/brazil/new-pics/belo-horizonte-3.jpg
  alt: Белу-Оризонти, столица штата Минас-Жерайс
gallery: []
themes:
  - culture
  - history
relatedDestinations:
  - destination_brazil_ouro_preto
  - destination_brazil_sao_paulo
  - destination_brazil_rio
featuredTours:
  - tour_brazil_gems_14d
featuredExcursions: []
facts:
  - value: Минас-Жерайс
    label: Белу-Оризонти является столицей штата
  - value: Пампулья
    label: один из основных архитектурных и прогулочных районов
  - value: Оскар Нимейер
    label: архитектурный сюжет города в исходном материале
  - value: Ору-Прету
    label: исторический блок, который удобно связывать с Белу-Оризонти
featureBands:
  - eyebrow: Архитектура
    title: Пампулья и Оскар Нимейер
    text: >-
      Старый материал Ada Tours выделяет Пампулью и проекты Оскара Нимейера как одну из главных причин добавить Белу-Оризонти в маршрут по Минас-Жерайс.
    image: https://brasiltours.ru/image/countries/brazil/new-pics/belo-horizonte-3.jpg
  - eyebrow: Маршрут
    title: Городской вход в Минас-Жерайс
    text: >-
      Белу-Оризонти удобно использовать как логистическую точку для Ору-Прету и других исторических мест штата, а затем продолжать маршрут внутренним перелетом.
    image: https://brasiltours.ru/image/countries/brazil/new-pics/belo-horizonte-3.jpg
sourceUrl: https://brasiltours.ru/ehkskursii-i-tury-v-gorod-belu-orizonte-v-brazilii
sourceSnapshot: page_texts_original/ehkskursii-i-tury-v-gorod-belu-orizonte-v-brazilii__b4f01d5a.md
updatedAt: 2026-09-25
---

Белу-Оризонти является столицей штата Минас-Жерайс. В старом материале Ada Tours город описан через зеленые зоны, музеи и архитектуру, при этом самый сильный визуальный сюжет связан с Пампульей и работами Оскара Нимейера.

Для большого маршрута по Бразилии Белу-Оризонти особенно полезен в связке с Ору-Прету. Город дает аэропорт и современную инфраструктуру, а историческая часть поездки продолжается уже в старых городах Минас-Жерайс.

В программе «Бразилия за 14 дней: Рио, Ору-Прету, Сальвадор, Прайя-ду-Форте и Игуасу» Белу-Оризонти используется как точка перелета после исторического блока. Это хороший пример его роли в маршруте: не отдельная длинная остановка, а связующее звено между регионами.
