---
id: destination_brazil_ouro_preto
locale: ru
name: Ору-Прету
slug: ouro-preto
status: approved
countryId: country_brazil
destinationType: city
summary: >-
  Ору-Прету в штате Минас-Жерайс вырос в эпоху золотой лихорадки. Сегодня сюда едут ради барочной архитектуры, старых шахт, музеев, площади Тирадентис и исторического центра, включенного в список Всемирного наследия UNESCO.
hero:
  src: https://brasiltours.ru/image/countries/brazil/new-pics/ouro-preto-2.jpg
  alt: Исторический центр Ору-Прету в штате Минас-Жерайс
gallery: []
themes:
  - culture
  - history
relatedDestinations:
  - destination_brazil_rio
  - destination_brazil_salvador
featuredTours:
  - tour_brazil_gems_14d
featuredExcursions: []
facts:
  - value: "1980"
    label: исторический центр в списке Всемирного наследия UNESCO
  - value: Минас-Жерайс
    label: штат на юго-востоке Бразилии
  - value: Барокко
    label: архитектура исторического центра
  - value: Золотые шахты
    label: часть истории города и экскурсионной программы
featureBands:
  - eyebrow: История
    title: Город золотой лихорадки
    text: >-
      Ору-Прету формировался вокруг добычи золота. Эта история читается в старой застройке, музеях и шахтах, часть которых открыта для посещения.
    image: https://brasiltours.ru/image/countries/brazil/new-pics/ouro-preto-2.jpg
  - eyebrow: Архитектура
    title: Площадь Тирадентис и барочный центр
    text: >-
      Центральная часть города соединяет площадь Тирадентис, музеи, церкви и крутые мощеные улицы. С верхних точек открываются виды на историческую застройку и окрестные холмы.
    image: https://brasiltours.ru/image/countries/brazil/new-pics/ouro-preto-2.jpg
sourceUrl: https://brasiltours.ru/ehkskursii-i-tury-v-gorod-oru-pretu-braziliya
sourceSnapshot: page_texts_original/ehkskursii-i-tury-v-gorod-oru-pretu-braziliya__226a4f58.md
updatedAt: 2026-09-25
---

Ору-Прету находится в штате Минас-Жерайс и тесно связан с эпохой добычи золота. В старом центре сохранились барочные церкви, музеи, площади и плотная застройка на холмистом рельефе.

Площадь Тирадентис остается одной из основных точек прогулки. От нее удобно продолжать маршрут по историческим улицам, музеям и религиозным памятникам. Отдельный блок поездки можно посвятить старым золотым шахтам, которые показывают другую сторону истории региона.

В большом маршруте по Бразилии Ору-Прету хорошо соединяется с Рио-де-Жанейро и дальнейшим перелетом через Белу-Оризонти. Такой формат позволяет добавить к мегаполисам и природным регионам отдельный исторический блок.
